...
/**
 * Sizes run one step larger than stock shadcn. HoopSync is a phone-first app
 * whose users are mostly on a court, and a 32px control is below the 44px
 * comfortable-tap floor. `default` is now 40px, `lg` 44px.
 *
 * Filled variants carry an inset top highlight plus a contact shadow rather
 * than sitting flat. It is a one-pixel detail and it is most of why a button
 * reads as a physical control instead of a coloured rectangle.
 */
const buttonVariants = cva(
  "group/button inline-flex shrink-0 items-center justify-center rounded-lg border border-transparent bg-clip-padding font-medium whitespace-nowrap transition-all outline-none select-none focus-visible:border-ring focus-visible:ring-3 focus-visible:ring-ring/50 active:not-aria-[haspopup]:translate-y-px disabled:pointer-events-none disabled:opacity-50 aria-invalid:border-destructive aria-invalid:ring-3 aria-invalid:ring-destructive/20 dark:aria-invalid:border-destructive/50 dark:aria-invalid:ring-destructive/40 [&_svg]:pointer-events-none [&_svg]:shrink-0 [&_svg:not([class*='size-'])]:size-4",
  {
    variants: {
      variant: {
        default:
          "bg-primary text-primary-foreground shadow-[inset_0_1px_0_0_oklch(1_0_0/0.14),0_1px_2px_-1px_var(--shadow-base),0_2px_6px_-3px_var(--shadow-base)] hover:bg-[color-mix(in_oklch,var(--primary),var(--foreground)_12%)] dark:shadow-[0_1px_2px_-1px_var(--shadow-base)]",
        brand:
          "bg-brand-strong text-brand-foreground shadow-[inset_0_1px_0_0_oklch(1_0_0/0.18),0_1px_2px_-1px_var(--shadow-base),0_4px_12px_-4px_color-mix(in_oklab,var(--brand)_50%,transparent)] hover:bg-[color-mix(in_oklch,var(--brand-strong),black_8%)] dark:hover:bg-[color-mix(in_oklch,var(--brand-strong),white_8%)]",
        outline:
          "border-border bg-card shadow-xs hover:bg-accent hover:text-foreground aria-expanded:bg-accent aria-expanded:text-foreground dark:border-input dark:bg-input/30 dark:hover:bg-input/50",
        secondary:
